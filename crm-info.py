...
from analysismethodprefs import all_analysis_methods as ALLMETHODS
...
class Crm:
	...
	def addChemistry(
		...
		# print(f'Trying to add chemistry: {chem_formula=}, {chem_concentration=}, {chem_unit=}, {chem_analysis_method=}, {chem_is_indicative=}')

		# make sure chem_concentration doesn't have symbols
		chem_concentration = str(chem_concentration).strip(" <>")

		# make sure chem_concentration is type float
		chem_concentration = float(chem_concentration)
...
def main():
	...
	catalogue_path = filedialog.askopenfilename(title="Please select the OREAS Catalogue CSV File:", initialdir=os.getcwd(),filetypes=[("CSV Files", "*.csv")])# "oreas-catalogue-2023-11-03.csv"
	cat_df = pd.read_csv(catalogue_path)

	indicatives_csv_path = filedialog.askopenfilename(title="Please select the Combined Indicative Values CSV File:", initialdir=os.getcwd(),filetypes=[("CSV Files", "*.csv")])
	indicatives_df = pd.read_csv(indicatives_csv_path)
	fnames = [cat_df,indicatives_df]
	combined_df = pd.concat(fnames, ignore_index=True)
	cat_df = combined_df
	...
	# add all non-prioritised analysis methods to bottom of each element's priority order. 
	# this ensures we get some kind of concentration data for an element, even if it is from a bad analysis method. 
	# chris requested this be added 2024/05/24
	for _element, _prefmethodlist in PREFS.items():
		# iterate through entire element method preference list and add method if absent (appending adds to end of list, hence lowest priority)
		print(f"Appending non-prioritised methods to priority list for element: {_element}")
		for _method in ALLMETHODS:
			if _method not in _prefmethodlist:
				_prefmethodlist.append(_method)
	...
	outputxlsxname = f"ALL_OREAS_CRM_DATA_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.xlsx"
